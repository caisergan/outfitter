...
**Project Goal**: Ship a mobile-first (iOS + Android) fashion app with Playground, Assistant, Wardrobe, and Discover tabs backed by a FastAPI + PostgreSQL + AI stack.
...
| 5 | Core Features Complete | Playground, Wardrobe, Assistant, Discover all functional |
...
- [ ] Create `wardrobe_items` ORM model with `user_id` FK, `clip_embedding VECTOR(512)`, `times_used`, `deleted_at` (soft delete)
  - Done: Model importable; migration generated
- [ ] Create `saved_outfits` ORM model with `slots JSONB`, `source` check constraint (`playground`/`assistant`), `generated_image_url`
  - Done: Model importable; migration generated
...
- [ ] Implement `POST /outfits` — save outfit to Lookbook, validate `source` is `playground` or `assistant`, validate slot item IDs exist
...
- [ ] Configure `go_router` with 4-tab `ShellRoute` (Discover, Playground, Assistant, Wardrobe) and auth guard redirecting unauthenticated users to login
...
## Phase 16: Flutter Playground Tab
...
- [ ] Build Lookbook sub-view: grid of saved outfits; tapping re-opens outfit in Playground with slots pre-filled
...
- [ ] Implement "Try On" button on outfit card — navigates to Playground tab with all outfit slots pre-filled
...
| 16 — Flutter Playground | 7 | After Phase 9, 12 + 14 |
