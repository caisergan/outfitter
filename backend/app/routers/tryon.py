...
def _serialize_run(row: TryOnRun) -> RunOut:
...
async def _count_today(db: AsyncSession, user_id: uuid.UUID) -> int:
    ...
    q = select(func.count(TryOnRun.id)).where(
        TryOnRun.user_id == user_id,
        TryOnRun.created_at >= midnight,
        TryOnRun.deleted_at.is_(None),
    )
    if not settings.TRYON_FAILED_RUNS_COUNT_TOWARD_CAP:
        q = q.where(TryOnRun.status != "failed")
    ...
    try:
        ...
        try:
            for idx, data in enumerate(images_bytes):
                key = f"tryon/{user_id}/{run_id}/{idx}.png"
...
async def _mark_run_finished(
    *,
    ...
    async with AsyncSessionLocal() as session:
        row = await session.get(TryOnRun, run_id)
        if row is None:
...
async def generate_image(
    ...
    if used >= settings.TRYON_DAILY_CAP:
        midnight = _utc_midnight_today()
        raise HTTPException(
            ...
            detail={
                ...
                "error": {
                    ...
                    "limit": settings.TRYON_DAILY_CAP,
                    "used": used,
    ...
    # 7. Return immediately — clients poll /tryon/runs/{run_id} until
    ...
    return GenerateResponse(
        ...
        elapsed_ms=0,
        daily_used=used + 1,
        daily_limit=settings.TRYON_DAILY_CAP,
    )
...
async def list_runs(
    ...
    q = (
        select(TryOnRun)
        .where(
            TryOnRun.user_id == current_user.id,
            TryOnRun.deleted_at.is_(None),
        )
        .order_by(TryOnRun.created_at.desc(), TryOnRun.id.desc())
    )
    if cursor:
        ts, rid = _decode_cursor(cursor)
        q = q.where(
            or_(
                TryOnRun.created_at < ts,
                and_(
                    TryOnRun.created_at == ts,
                    TryOnRun.id < rid,
                ),
