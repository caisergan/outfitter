...
export default function TryOnPage() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            .catch((err) => {
                if (cancelled) return;
                console.error("Failed to load try-on config:", err);
                toast.error(
                    "Could not load try-on config; using local defaults",
                );
                setTemplates([]);
                setAllPersonas([]);
            });
    ...
    async function handleGenerate() {
        ...
        try {
            ...
            const accepted = await generateTryOnImage({
            ...
            while (Date.now() - startedAt < POLL_CEILING_MS) {
                run = await fetchTryOnRun(accepted.run_id);
    ...
    async function loadMoreRuns() {
        ...
        setRunsLoading(true);
        try {
            const data = await fetchTryOnRuns({
                limit: 10,
                cursor: runsCursor,
            });
    ...
    async function refreshRunsAfterGenerate() {
        try {
            const data = await fetchTryOnRuns({ limit: 10 });
            setRuns(data.items);
            setRunsCursor(data.next_cursor);
        } catch (err) {
    ...
    async function submitSaveAsTemplate() {
        ...
        try {
            const created = await createTryOnTemplate({
                slug,
            ...
            const fresh = await fetchTryOnTemplates();
            setTemplates(fresh);
    ...
    function downloadImage(dataUrl, index) {
        ...
        link.href = dataUrl;
        link.download = `try-on-${Date.now()}-${index}.png`;
        link.click();
    }
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-slate-400 mt-1">
                    ...
                    Generation history is saved to try-on runs.
                </p>
            </div>
